...
Like the package? Please give it a 👍 here: https://pub.dev/packages/saluone_native_splash
''';
...
/// Get config from `pubspec.yaml` or `saluone_native_splash.yaml`
Map<String, dynamic> getConfig({
  ...
  // if `saluone_native_splash.yaml` exists use it as config file, otherwise use `pubspec.yaml`
  String filePath;
  if (configFile != null) {
    ...
    filePath = 'saluone_native_splash-${_flavorHelper.flavor}.yaml';
  } else if (File('saluone_native_splash.yaml').existsSync()) {
    filePath = 'saluone_native_splash.yaml';
  } else {
  ...
  if (yamlMap['saluone_native_splash'] is! Map) {
    throw Exception(
      'Your `$filePath` file does not contain a '
      '`saluone_native_splash` section.',
    );
  }

  // yamlMap has the type YamlMap, which has several unwanted side effects
  return _yamlToMap(yamlMap['saluone_native_splash'] as YamlMap);
}
  ...
  for (final MapEntry<dynamic, dynamic> entry in yamlMap.entries) {
    if (!_Parameter.all.contains(entry.key)) {
      ...
      print(pen("⚠️ The parameter \"${entry.key}\" was found "
          "in your saluone_native_splash config, but \"${entry.key}\" "
          "is not a valid saluone_native_splash parameter."));
      exit(1);
    }
